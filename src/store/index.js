...
/*路径漫游*/
    ...
    FLYTOMODEL(state,name){
        ...
        // 时间速率，数字越大时间过的越快
        viewer.clock.multiplier = 20  //拓展，速度控制
        // 循环执行 
    ...
    //退出飞行状态
    EXITFLYING(state){
        this.dispatch('clearArrAll')
        state.isFlying=false
        state.theFlyStopTime=null
        state.flyExection=undefined
    },
    //飞行暂停
    MOVENPAUSE(){
  ...
  actions: {
    /* 地图标签*/
    addViewPoint(context,pointName){
        context.commit("ADDVIEWPOINT",pointName)
    },
    deleteViewPoint(context,guid){
        context.commit("DELETEVIEWPOINT",guid)
    },
    flyToPoint(context,rowData){
        context.commit("FLYTOPOINT",rowData)
    },
    //日期格式化
